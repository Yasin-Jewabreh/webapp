...
with app.app_context():
    ...
    if not Nutzer.query.first():
        ...
        termin1 = Termin(
            ...
            titel="Gartenarbeit bei Oma", notizen="Rasenmäher steht im Schuppen",
            datum=date(2026, 7, 20), uhrzeit_beginn=time(14, 0), uhrzeit_ende=time(16, 0),
            complete=False
        )
        termin2 = Termin(
            helfer_id=test_nutzer.id, pp_id=test_nutzer.id, auftrag_id=test_auftrag.id,
            titel="Einkaufen fahren", notizen="Einkaufszettel liegt auf dem Küchentisch",
            datum=date(2026, 6, 15), uhrzeit_beginn=time(10, 0), uhrzeit_ende=time(11, 30),
            complete=True
        )
        db.session.add_all([termin1, termin2])
        db.session.commit()
